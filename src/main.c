...
static void inbox_received_callback(DictionaryIterator *iterator, void *context) {
  ...
  while(t != NULL) {
    ...
    switch(t->key) {
      case KEY_TEMPERATURE:
        APP_LOG(APP_LOG_LEVEL_ERROR, "setting temp with %s", t->value->cstring);
        //snprintf(temperature_buffer, sizeof(temperature_buffer), "%s\u00B0", t->value->cstring);
        break;
      case KEY_CONDITIONS:
        APP_LOG(APP_LOG_LEVEL_ERROR, "setting conditions with %s", t->value->cstring);
        //snprintf(conditions_buffer, sizeof(conditions_buffer), "%s", t->value->cstring);
        break;
      case KEY_CITY:
        APP_LOG(APP_LOG_LEVEL_ERROR, "setting city with %s", t->value->cstring);
        text_layer_set_text(s_weather_city_layer, t->value->cstring);    
        break;
      case KEY_SYMBOL:
        APP_LOG(APP_LOG_LEVEL_ERROR, "setting symbol with %s", t->value->cstring);
        text_layer_set_text(s_symbol_layer, t->value->cstring);  
        break;
      case KEY_PRICE:
        APP_LOG(APP_LOG_LEVEL_ERROR, "setting price with %s", t->value->cstring);
  ...
  {
    APP_LOG(APP_LOG_LEVEL_ERROR, "setting weather with %s", temperature_buffer);
